...
| Dev Agent GitHub App | `fabric-agentic-dev-agent`, App ID `4672750`, Installation ID `155470382`; installata sul solo repository previsto. La private key PEM è attesa fuori dal repo in `%USERPROFILE%\.fabric-agentic\dev-agent\github-app-private-key.pem`, directory ACL limitata all'owner |
...
| Runtime Dev Agent | Claude Code `2.1.228` installato nativamente su Windows, canale `stable`. Verificata sessione headless `claude -p` con esito `READY` il 2026-08-21; Azure DevOps non interattivo è verificato. Restano il clone isolato e la private key locale della GitHub App per push/PR e trigger C prima del dispatcher completo |
