...
| **Attempt** | Un singolo tentativo di lavorare un work item. Ha `attempt_id`, work item, `source_revision`, stato, lease e `retry_count`. Un work item può avere più tentativi; il tentativo, non il work item, è l'unità di evidenza. |
| **Attempt ledger** | File `attempts.json` nel perimetro locale dell'agente (`fabric_agentic/attempt_ledger.py`) che registra ogni tentativo. Contiene solo identificativi, stati e istanti: mai token, credenziali o contenuto del work item. |
| **Claim** | Presa in carico atomica di un work item da parte di un dispatcher, serializzata da un lock file. Due dispatcher concorrenti non possono ottenere lo stesso work item. |
| **Lease** | Scadenza del claim. Un dispatcher che muore non blocca il ticket: alla scadenza il tentativo è `expired` e il ciclo successivo lo riprende. Viene rinnovato mentre la sessione gira; default 900 s (`agent.lease_seconds`). |
...
| Layout locale degli agenti | Ogni agente vive in `~/.fabric-agentic/<agente>-agent/` con `dispatcher-config.json`, `github-app-private-key.pem`, `repository/`, `state.json` e `tasks/`; il Dev Agent tiene qui anche `attempts.json`, il ledger dei tentativi. `python -m fabric_agentic doctor` verifica il layout e stampa il comando di avvio; vedi `docs/technical/12-console-e-avvio.md` |
...
| Framework CRM porting | Fonte pulita `fabric-universal-connector` commit `3303149c809172d0d320bfec353b5d81a`, registrata in `PROVENANCE.md`. Configurazione, schema, request builder, staging per run, PK check, merge Bronze, audit per `run_id`, watermark post-audit e rail `run_load` sono implementati e coperti da test. Restano aperti: `reconciliation` calcolata invece che asserita, evidenza Fabric legata al run, e la sequenza run iniziale/idempotente/delta con verifica SQL. Inventario anello per anello in `docs/technical/14-inventario-catena-crm-accounts.md` |
...
| Runtime dei tre agenti — **BLOCCANTE per cliente/collega** | Issue, Dev e Review invocano oggi lo stesso `claude` (Claude Code) sotto l'account **personale** dell'operatore (`configuration/*.json`, campo `claude_command`). Non è provisioning multiplo: è un'unica identità di inferenza condivisa da tutti e tre. Prima di usare il flusso su un cliente o di farlo eseguire a un collega, questa identità deve diventare aziendale — vedi prerequisiti in `docs/functional/06-onboarding-nuovo-cliente.md` §3.1 e la voce Q-13 del PRD. Il PRD prevedeva inoltre un vendor diverso per il Review Agent (indipendenza del giudizio): oggi non è implementato, gira sullo stesso account Claude Code del Dev Agent |
