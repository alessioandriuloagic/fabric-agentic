...
| Token GitHub Dev Agent | `fabric_agentic/github_app_auth.py` firma un JWT valido nove minuti e ottiene un installation token solo in memoria, ed è richiamabile come prima da `python -m scripts.github_app_auth`. Il comando `verify` può mostrare i repository autorizzati, mai token o contenuto PEM |
...
| Token GitHub Review Agent | `scripts/review_vote_publish.py` conia l'installation token al momento della pubblicazione, riusando `fabric_agentic/github_app_auth.py`, e lo tiene solo in memoria di processo. La sessione di review non conia token, non firma il JWT e non conosce il percorso della key |
